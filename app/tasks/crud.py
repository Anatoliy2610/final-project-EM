...
from app.tasks.utils import check_absence_task, check_availability_task, check_executor, check_job_evaluation, check_user_admin
...
class TaskCRUD:
    ...
    async def get_tasks_db(self, user):
        try:
            query = await self.db.execute(
                select(TaskModel).filter(TaskModel.team_id == user.team_id)
                .options(
            ...
            raise HTTPException(
                status_code=500,
                detail="Ошибка при получении данных о задачах"
            )
    ...
    async def add_task_db(self, user, data_task):
        try:
            try:
                ...
                logger.warning(f"Некорректный формат даты дедлайна: {data_task.dedline}")
                raise HTTPException(
                    status_code=400,
            detail="Некорректный формат даты дедлайна"
                )
            db_task = TaskModel(
                name=data_task.name,
    ...
    async def get_data_for_update(self, task_id, user_data):
        try:
            query_task = await self.db.execute(select(TaskModel).filter(
                    TaskModel.id == task_id,
                    TaskModel.team_id == user_data.team_id
                ))
            task = query_task.scalars().first()
            query_team = await self.db.execute(select(UserModel).filter(
    ...
    async def update_task_db(self, user, data_task):
        try:
            query = await self.db.execute(select(TaskModel).filter(
    ...
    async def get_tasks_user_db(self, executor_id):
        try:
            avg_result = await self.db.execute(
                select(func.avg(TaskModel.job_evaluation))
                .filter(
            ...
            tasks_result = await self.db.execute(
                select(TaskModel)
                .filter(TaskModel.executor_id == executor_id)
            )
    ...
    async def get_task_user_db(self, task_id):
        try:
            query = await self.db.execute(select(TaskModel).filter(TaskModel.id == task_id).options(
                selectinload(TaskModel.executor),
                selectinload(TaskModel.team),
