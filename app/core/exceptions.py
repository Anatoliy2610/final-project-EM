...
class ExceptionService:
    ...
    async def check_user(self, user_data):
        query = await self.db.scalars(select(UserModel).filter(UserModel.email == user_data.email))
        user = query.first()
        if user:
            raise HTTPException(
    ...
    async def check_meeting(self, data_meeting):
        query_meeting = await self.db.execute(select(MeetingModel).filter(
    ...
    async def check_executor(self, data_task, user):
        query_executor = await self.db.execute(select(UserModel).filter(
    ...
    async def check_availability_team(self, data_team):
        query = await self.db.execute(select(TeamModel).filter(TeamModel.name == data_team.name))
    ...
    async def check_absence_user(self, data_user):
        query = await self.db.execute(select(UserModel).filter(UserModel.email == data_user.email_user))
